...
EtcdSnowflake::EtcdSnowflake() {
    const char* etcd_host = getenv("ETCD_SERVICE_HOST") ? getenv("ETCD_SERVICE_HOST") : "etcd";
    const char* etcd_port = getenv("ETCD_SERVICE_PORT") ? getenv("ETCD_SERVICE_PORT") : "2379";
...
uint64_t EtcdSnowflake::claim_node_id() {
    ...
    for (int i = 0; i <= MAX_NODE_ID; ++i) {
        ...
        // For simplicity in this example, we'll just use a shell command to base64 encode
        // In a real production app, use a proper base64 library
        char key_b64[256];
        snprintf(key_b64, sizeof(key_b64), "echo -n '%s' | base64 -w 0", key.c_str());
        ...
        // Use etcd transaction to only put if the key doesn't exist (create_revision == 0)
        string txn_url = etcd_endpoint + "/kv/txn";
        stringstream txn_req;
        txn_req << R"({
            "compare": [{"target": "CREATE", "key": ")" << encoded_key << R"(", "createRevision": 0}],
            "success": [{"requestPut": {"key": ")" << encoded_key << R"(", "value": "MQ==", "lease": ")" << lease_id << R"("}}]
        })";
    ...
    throw runtime_error("Failed to claim any Node ID from etcd (all 1024 IDs in use)");
}
